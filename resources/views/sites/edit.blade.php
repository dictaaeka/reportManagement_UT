        ...
        <div class="data-management-header">
            <div class="reports-header-content flex items-center justify-between gap-6 p-5 sm:p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="data-management-header-title">{{ __('Edit Site') }}</h2>
                        <p class="data-management-header-description">Perbarui detail lokasi agar tetap konsisten dengan laporan.</p>
                    </div>
                </div>
            ...
            <div class="data-management-card">
                <div class="data-management-card-header">
                    <div class="data-management-card-header-inner">
                        ...
                        <div>
                            <h3 class="data-management-card-title">Perbarui Site</h3>
                            <p class="data-management-card-description">Ubah informasi payment site tanpa mengubah logika aplikasi.</p>
                        </div>
                    </div>
                </div>

                <div class="data-management-card-body">
                    <form action="{{ route('sites.update', $site) }}" method="POST" class="data-management-form-grid">
                        @csrf
                        @method('PUT')

                        <div class="data-management-field">
                            <label for="name" class="data-management-label">Nama</label>
                            <input id="name" type="text" name="name" value="{{ old('name', $site->name) }}" class="data-management-input" required>
                            @error('name')
                        ...
                        <div class="data-management-field">
                            ...
                            <input id="location" type="text" name="location" value="{{ old('location', $site->location) }}" class="data-management-input">
                            @error('location')
                            <p class="data-management-error">{{ $message }}</p>
                            @enderror
                        </div>
